...
> [!TIP]
> If you want to add new mods to an already running server in the future, stop it using the command `docker stop terraria-server`, add the new mods to `mods.txt`, and restart the container with the command `./scripts/install.sh` while in the root directory of the project.
